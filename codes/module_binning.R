...
binning_server <- function(input, output, session, modeltraintest,
  ...
  binning_vars <- reactive({
    ...
    binvars_nc <- mds$vnames[mds$vtypes == "numeric" | mds$vtypes == "integer" | mds$vtypes == "character"]
    ...
    if (binvar_type == "numeric" || binvar_type == "integer") {
      withProgress({
        if (input$bintype == "CIT") {
          ...
          # user inputs needs to be split from text input, any non-space delimiters will currently break
          cuts2 <- as.numeric(unlist(strsplit(input$input_cut, " ")))
          b <- binning_numeric(binvar, goodbad_var(), mtt$train, mtt$test, type = input$bintype, cuts = cuts2) #, cuts = c(input$input_cut))
                }
        binning_g[[binvar]] <- b
  ...
  observeEvent(input$docombine, {
    ...
    if (binvar_type == "numeric" || binvar_type == "integer") {
      ...
      withProgress({
        b <- binning_numeric(binvar, goodbad_var(), mtt$train, mtt$test, cuts = cts)
        binning_g[[binvar]] <- b
  ...
  observeEvent(input$dosplit, {
    ...
    if (binvar_type == "numeric" || binvar_type == "integer") {
      ...
      withProgress({
        ...
        setProgress(value = 1, message = "Binning complete.")
        Sys.sleep(1)
      }, value = 0.5, message = paste0("Binning ", binvar, ". Please wait."))
    } else if (binvar_type == "character") {
      withProgress({
        grps <- bin$bin_train$groups
        ...
        b <- binning_categorical(binvar, goodbad_var(), mtt$train, mtt$test, grps)
        binning_g[[binvar]] <- b
        setProgress(value = 1, message = "Binning complete.")
        Sys.sleep(1)
      }, value = 0.5, message = paste0("Binning ", binvar,
