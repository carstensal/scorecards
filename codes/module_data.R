...
#'     logical to identify a particular variable as the Target Variable,
...
data_server <- function(input, output, session) {
  ...
  output$modeldataspecs <- renderRHandsontable({
    ...
    rhandsontable(modeldataspecs(),
                  ...
                  stretchH = "all",
                  rowHeaders = NULL,
                  colHeaders = c("Variable", "Type", "Target Variable",
                                 "Stratification", "Bin")) %>%
      hot_cols(columnSorting = FALSE) %>%
      hot_col("Variable", readOnly = TRUE) %>%
      hot_col("Target Variable", renderer = bool_renderer) %>%
